...
export default function BusinessesCreditCard() {
  const [creditCardDataArray, setCreditCardDataArray] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const { t } = useLanguage();

  useEffect(() => {
    // Dynamically import businessesCreditCard data from data.js
    import(`../../data.js`)
      .then((dataModule) => {
        const businessesCreditCardData = (dataModule as any)
          .businessesCreditCard;
        if (Array.isArray(businessesCreditCardData)) {
          setCreditCardDataArray(businessesCreditCardData);
        } else {
      ...
      .catch((error) => {
        console.error("Error loading businessesCreditCard data:", error);
        setCreditCardDataArray([]);
        setLoading(false);
      });
  }, []);
      ...
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            {t("no.data.found.for")} {t("business.credit.card")}
          </p>
        </div>
      </div>
    );
      ...
      <main className="w-full px-6 py-8">
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className="bg-blue-600 text-white px-3 py-1 rounded text-xs font-medium">
            {t("business.credit.cards").toUpperCase()}
          </span>
        </div>
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">
            {t("business.credit.cards.title")}
          </h1>
          <p className="text-gray-700">
            {t("business.credit.cards.description")}
          </p>
          <div className="mt-4 text-sm text-gray-600">
            {t("showing")} {creditCardDataArray.length} {t("of")}{" "}
            {creditCardDataArray.length} {t("business.credit.cards")}
          </div>
        </div>
                ...
                >
                  ...
                  <div className="flex items-start gap-4 py-4 px-6">
                    <img
                      ...
                      alt={
                        creditCard.reviews?.head || t("business.credit.card")
                      }
                  ...
                  <div className="py-4 px-6">
                    ...
                    <div className="relative">
                      ...
                      <div className="flex justify-between text-xs text-gray-500 mt-2">
                        ...
                        <span>{creditCard.purchaseApr?.max || "28"}%</span>
                      </div>
                    </div>
                  </div>
                          ...
                          style={{
                            width: `${Math.min(
                              ((creditCard.balanceTransferApr?.maxValue || 0) /
                                (creditCard.balanceTransferApr?.max || 27)) *
                                100,
                              100
                            )}%`,
                          }}
