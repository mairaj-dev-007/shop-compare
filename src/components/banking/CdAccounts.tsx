...
export default function CdAccounts() {
  ...
  useEffect(() => {
    // Dynamically import cdAccounts data from data.js
    import(`../../data.js`)
      .then((dataModule) => {
        const cdAccountsData = (dataModule as any).cdAccounts;
        if (Array.isArray(cdAccountsData)) {
          setBankingDataArray(cdAccountsData);
        } else {
          setBankingDataArray([]);
        }
        setLoading(false);
      ...
      .catch((error) => {
        console.error("Error loading cdAccounts data:", error);
        setBankingDataArray([]);
        setLoading(false);
      });
  }, []);
  ...
  if (!bankingDataArray || bankingDataArray.length === 0) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            {t("no.data.found.for")} {t("cd.account")}
          </p>
        </div>
      </div>
    );
      ...
      <main className="w-full px-6 py-8">
        ...
        {/* Table Headers - EXACTLY as in the image */}
        <div className="w-full overflow-x-auto">
          <div className="min-w-[1200px] grid grid-cols-6 gap-0 border-b border-gray-200 text-xs font-bold text-gray-600 uppercase tracking-wide mb-2">
            ...
            <div className="py-2 px-4">{t("minimum.deposit")}</div>
            <div className="py-2 px-4">{t("cd.term")}</div>
            ...
            {bankingDataArray.map((item, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 py-4 px-4">
                  <img
                    src={item.image}
                    alt={item.reviews?.head || t("cd.account")}
                ...
                <div className="py-4 px-4">
                  <div className="font-bold text-gray-800 text-lg">
                    {item.minimumOpeningDeposit || "N/A"}
                  </div>
                ...
                {/* CERTIFICATE TERM */}
                <div className="py-4 px-4">
                  {item.certificateTerm &&
                  Array.isArray(item.certificateTerm) ? (
                    <div className="space-y-1">
                      {item.certificateTerm.map(
                        (term: any, termIdx: number) => (
                          <div
                            key={termIdx}
