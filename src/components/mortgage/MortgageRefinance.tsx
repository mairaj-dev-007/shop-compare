...
export default function MortgageRefinance() {
  ...
  useEffect(() => {
    ...
    import(`../../data.js`)
      ...
      .catch((error) => {
        console.error("Error loading mortgageRefinance data:", error);
        setMortgageDataArray([]);
        setLoading(false);
      });
  }, []);
      ...
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            {t("no.data.found.for")} mortgage refinance
          </p>
        </div>
      </div>
    );
      ...
      <main className="w-full px-6 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">
            {t("mortgage.refinance.title")}
          </h1>
          <p className="text-gray-700">{t("mortgage.refinance.description")}</p>
          <div className="mt-4 text-sm text-gray-600">
            {t("showing")} {mortgageDataArray.length} {t("of")}{" "}
            {mortgageDataArray.length} {t("mortgage.refinance.product")}
          </div>
        </div>
