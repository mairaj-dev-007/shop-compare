...
export default function HomeEquityLoan() {
  ...
  useEffect(() => {
    ...
    import(`../../data.js`)
      ...
      .catch((error) => {
        console.error("Error loading homeEquityLoan data:", error);
        setMortgageDataArray([]);
        setLoading(false);
      });
  }, []);
      ...
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            {t("no.data.found.for")} home equity loans
          </p>
        </div>
      </div>
    );
      ...
      <main className="w-full px-6 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">
            {t("home.equity.loans.title")}
          </h1>
          <p className="text-gray-700">{t("home.equity.loans.description")}</p>
          <div className="mt-4 text-sm text-gray-600">
            {t("showing")} {mortgageDataArray.length} {t("of")}{" "}
            {mortgageDataArray.length} {t("home.equity.loan")}
          </div>
        </div>
            ...
            <div className="min-w-[1200px] divide-y divide-gray-100">
              {mortgageDataArray.map((mortgage, index) => (
                ...
                >
                  ...
                  <div className="py-4 px-6">
                    <div className="font-bold text-gray-800 text-lg mb-3">
                      {mortgage.apr?.minValue && mortgage.apr?.maxValue
                        ? `${mortgage.apr.minValue}% - ${mortgage.apr.maxValue}%`
                        : t("n.a")}
                    </div>
                    <div className="relative">
